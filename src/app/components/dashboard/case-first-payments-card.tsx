...
async function getData(): Promise<SearchResponse<Case>> {
  const query = `status=${CaseStatus.ONGOING}`;
...
export async function CaseFirstPaymentsCard() {
  ...
  function getTitle() {
    return (
      <div className="flex p-2">
        ...
        <h3 className="ml-2 text-sm font-medium">Primeiros Pagamentos</h3>
      </div>
    );
  }
  ...
  return (
    <Card title={getTitle()}>
      ...
      <div className='mt-4 pl-1 flex items-center justify-start'>
        <Link
          href={`/cases?status=${CaseStatus.ONGOING}&page=1`}
