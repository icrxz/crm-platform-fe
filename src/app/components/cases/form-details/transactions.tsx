...
export function TransactionStatusForm({ crmCase }: TransactionStatusFormProps) {
  ...
  const [caseTransactions, setCaseTransactions] =
    useState<TransactionForm[]>(mappedTransactions);

  async function onSubmit() {
    setLoadingSubmit(true);

    try {
      const transactions: CreateTransaction[] = caseTransactions.map(
        (transaction) => {
          return {
      ...
      const transactionResp = await createTransactions(
        crmCase.case_id,
        transactions
      );
      ...
      const statusResp = await changeStatus(
        crmCase.case_id,
        CaseStatus.RECEIPT
      );
  ...
  return (
    <Card title="Adicionar transações" titleSize="xl">
      <form action={dispatch} className="px-5">
        ...
        <div className="pl-1">
          <Button isLoading={loadingSubmit}>Concluir</Button>
        </div>
      </form>
    </Card>
  );
