...
export default function CaseDetails({ crmCase, userRole }: CaseDetailsProps) {
  ...
  const caseStatusList = Object.values(caseStatusMap).filter(
    (status) => status !== caseStatusMap['Canceled']
  );
  ...
  function getStatusColor(
    ...
    if (caseStatus === caseStatusMap[actualStatus]) {
      if (
        caseStatus === caseStatusMap[CaseStatus.ONGOING] &&
        isBeforeTargetDate
      ) {
        ...
        <div className="h-fill w-1/3">
          <Card title="Dados do caso" titleSize="xl">
            <div className="ml-4 items-center gap-8">
              ...
              <CardText
                title="Seguradora:"
                text={crmCase.contractor?.company_name || ''}
              />
              {crmCase.metadata?.category && (
                <CardText
                  title="Categoria:"
                  text={
                    caseCategoryMap[
                      crmCase.metadata.category as CaseCategory
                    ] || crmCase.metadata.category
                  }
                />
              )}
              <CardText
                ...
                text={parseDateTime(crmCase.created_at)}
              />
              <CardText title="Criado por:" text={crmCase.created_by} />
              <CardText
                title="Prioridade:"
                text={casePriorityMap[crmCase.priority]}
              />
              ...
              <CardText
                ...
                text={parseDateTime(crmCase.due_date, 'dd/MM/yyyy')}
              />
              {crmCase.partner && (
                <CardText
                  title="Técnico:"
                  text={`${crmCase.partner.first_name} ${crmCase.partner.last_name}`}
                />
              )}
              ...
              {crmCase.status == CaseStatus.ONGOING && crmCase.target_date && (
                <CardText
                  title="Data agendada:"
                  text={parseDateTime(crmCase.target_date, 'dd/MM/yyyy')}
                />
              )}
        ...
        <div className="h-fill w-1/2">
          <Card title="Cliente" titleSize="xl">
            <div className="ml-4 items-center gap-8">
              ...
              <CardText
                title="Documento:"
                text={parseDocument(crmCase.customer?.document || '')}
              />
              <CardText
                title="Endereço:"
                text={crmCase.customer?.shipping.address || ''}
              />
              <CardText
                title="Cidade:"
                text={crmCase.customer?.shipping.city || ''}
              />
              <CardText
                title="Estado:"
                text={crmCase.customer?.shipping.state || ''}
              />
              <CardText
                title="CEP:"
                text={crmCase.customer?.shipping.zip_code || ''}
              />
              <CardText
                title="Telefone:"
                text={crmCase.customer?.personal_contact?.phone_number || ''}
              />
              <CardText
                title="Email:"
                text={crmCase.customer?.personal_contact?.email || ''}
              />
            </div>
          </Card>
        </div>
          ...
          <Card title="Produto" titleSize="xl">
            <div className="ml-4 items-center gap-8">
              <CardText
                ...
                text={crmCase.product?.product_name || ''}
              />
              <CardText
                title="Descrição:"
                text={crmCase.product?.product_description || ''}
              />
              <CardText title="Marca:" text={crmCase.product?.brand || ''} />
              <CardText title="Modelo:" text={crmCase.product?.model || ''} />
              <CardText
                title="Valor:"
                text={parseToCurrency(crmCase.product?.value || 0)}
              />
              <CardText
                title="Número de série:"
                text={crmCase.product?.serial_number || ''}
              />
            </div>
          </Card>
        </div>
