  ...
  async function onSubmit(_: unknown, formData: FormData) {
    try {
      ...
      const transactionMO = {
        ...
        value: parseCurrencyToNumber(
          formData.get('amount_mo')?.toString() || ''
        ),
      };
      ...
      const transactionTransport = {
        transactionID:
          formData.get('transaction_id_transport')?.toString() || '',
        value: parseCurrencyToNumber(
          formData.get('amount_transport')?.toString() || ''
        ),
      };
      ...
      const transactionParts = {
        ...
        value: parseCurrencyToNumber(
          formData.get('amount_parts')?.toString() || ''
        ),
      };
      ...
      <form action={dispatch} className="space-y-4">
        ...
        <div className="flex flex-col items-center gap-4">
          <div>
            ...
            <input
              type="hidden"
              id="transaction_id_mo"
              name="transaction_id_mo"
              value={transaction?.mo.transaction_id}
            />
          </div>
          ...
          <div>
            ...
            <input
              type="hidden"
              id="transaction_id_transport"
              name="transaction_id_transport"
              value={transaction?.transport.transaction_id}
            />
          </div>
          ...
          <div>
            ...
            <input
              type="hidden"
              id="transaction_id_parts"
              name="transaction_id_parts"
              value={transaction?.parts.transaction_id}
            />
          </div>
        </div>
